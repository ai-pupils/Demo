...
const showThisMenuEle = (item: { meta: { access: any[] } }, access: any[]) => {
 ...
 * @param homeRoute
 */
export const getBreadCrumbList = (route: Route, homeRoute: any) => {
...
export const showTitle = (item: any, vm: any) => {
  ...
  if (useI18n) {
    if (title.includes('{{') && title.includes('}}') && useI18n)
      title = title.replace(/({{[\s\S]+?}})/, (m: any, str: any) =>
        str.replace(/{{([\s\S]*)}}/, (m: any, _: any) => vm.$t(_.trim()))
      );
 ...
 * @param {*} route 路由列表
 */
const hasAccess = (access: any, route: any) => {
 ...
 */
export const canTurnTo = (name: any, access: any, routes: any) => {
 ...
 */
export const getTableDataFromArray = (array: any) => {
  let columns = [];
  let tableData = [];
  if (array.length > 1) {
    ...
    tableData = array.map((item: any) => {
      let res: any = {};
      item.forEach((col: any, i: any) => {
        res[titles[i]] = col;
      });
      return res;
    });
...
export const findNodeUpper = (ele: any, tag: any): any => {
  if (ele.parentNode) {
...
export const findNodeUpperByClasses = (ele: any, classes: any): any => {
  let parentNode = ele.parentNode;
  if (parentNode) {
    let classList = parentNode.classList;
    if (
      classList &&
      classes.every((className: any) => classList.contains(className))
    ) {
      return parentNode;
    } else {
...
export const findNodeDownward = (ele: any, tag: any): any => {
