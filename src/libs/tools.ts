export const forEach = (
  arr: any[],
  fn: (currentValue: any, index: number, array: any[]) => void
) => {
 ...
 */
const getDate = (timeStamp: number, startType?: string) => {
...
export const getRelativeTime = (timeStamp: number | string) => {
  ...
  // 如果是毫秒格式则转为秒格式
  if (IS_MILLISECOND) Math.floor(((<number>timeStamp) /= 1000));
  // 传入的时间戳可以是数值或字符串类型，这里统一转为数值类型
  timeStamp = Number(timeStamp);
  // 获取当前时间时间戳
  const currentTime = Math.floor(Date.parse(String(new Date())) / 1000);
  // 判断传入时间戳是否早于当前时间戳
...
export const on: (
  ...
  if (document.addEventListener) {
    ...
    return function(
      element: { attachEvent: (arg0: string, arg1: any) => void },
      event: string,
      handler: any
    ) {
      if (element && event && handler) {
...
export const off: (
  ...
  if (document.removeEventListener) {
    ...
    return function(
      element: { detachEvent: (arg0: string, arg1: any) => void },
      event: string,
      handler: any
    ) {
      if (element && event) {
