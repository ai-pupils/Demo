<template lang="pug">
  section
    p.text-center.font18.mb20.mt20 信号强度（RSRP）
    Form.ml50
      FormItem
...
import { CabinetSignalsMonitorChartsOptions } from '@/const/data/cabinet';
import {
...
export default class SignalsMonitor extends mixins(LoadPageMixin, SearchMixin)
  ...
  isMounted = false;
  chartsOptions = { ...CabinetSignalsMonitorChartsOptions };
  // 限制搜索范围7天
  limit = 7;
  timeRadio = 'today';
  ...
  stateChanged(state: any) {
    if (state.isFulfill) {
      ...
      data.forEach((e: any) => {
        series[0].data.push([e.time, e.signal_strength]);
      });
