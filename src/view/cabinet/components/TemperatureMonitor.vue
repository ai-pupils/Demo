<template lang="pug">
  section
    p.text-center.font18.mb20.mt20 温度监控
    Form.ml50
      FormItem
...
import { CabinetTemperatureMonitorChartsOptions } from '@/const/data/cabinet';
import { elParseDaterangepickerOption, parseDateRange } from '@/utils/misc';
...
export default class TemperatureMonitor
  extends mixins(LoadPageMixin, SearchMixin)
  ...
  isMounted = false;
  chartsOptions = { ...CabinetTemperatureMonitorChartsOptions };
  // 限制搜索范围7天
  limit = 7;
  timeRadio = 'today';
  ...
  stateChanged(state: any) {
    ...
    if (state.isFulfill) {
      ...
      data.forEach((e: any) => {
        series[0].data.push([e.time, e.temperature]);
      });
