...
import { getIngressionGuideDispatch$,
  newIngressionGuideDispatch$,
  NewIngressionGuideDispatchParams,
  getOneGuide$,
...
export default class Dispatch extends mixins(LoadPageMixin)
  ...
  defaultParams = {}
  defaultApi: DefaultApiType = getIngressionGuideDispatch$;
  ...
  addNewDispatch(formData: NewIngressionGuideDispatchParams) {
    ...
    getOneGuide$({guideId}).subscribe(state => {
      if(!state.isError) {
        ...
        if(info.status === GUIDE_STATUS_CONFIRMED) {
          newIngressionGuideDispatch$(formData).subscribe(state => {
            if(!state.isError) {
