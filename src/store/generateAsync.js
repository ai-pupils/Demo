...
export const generateAction = ({mutateType, meta}) => (...arg) => {
  const [{commit}, payload] = arg;
  commit(mutateType, {payload, meta});
};

export const generateAsyncAction = (action, {mutateType, meta}) => (...arg) => {
  const [{commit}, ...payload] = arg;
  commit(`${mutateType}_${PENDDING_STATE}`, {payload, meta});
  return action(...payload)
    .then(response => {
      commit(`${mutateType}_${FULFILL_STATE}`, {response, payload, meta});
      return response;
    })
    .catch(error => {
      commit(`${mutateType}_${REJECTED_STATE}`, {error, payload, meta});
    });
};

export const generateMutation = (mutation, {mutateType, statePath}) => ({
  [mutateType]: (state, currentState) => {
    const payload = {...state[statePath].payload, ...currentState.payload};
    Vue.set(state, statePath, {payload});
  }
});
...
export const generateAsyncMutation = (mutation, {
  ...
  const errorFn = _.isFunction(mutation.failure) ? mutation.failure : (state, {error}) => {
    const currentState = mergeBy({
      isFetching: false,
      isError: true,
      error,
  ...
  if (_.isFunction(mutation)) {
    ...
    fulfillFn = _.isFunction(mutation.success) ? mutation.success : (state, {response}) => {
      const currentState = mergeBy({
        payload: response,
        isFetching: false,
        isError: false,
